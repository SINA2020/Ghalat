...
local json = (loadfile "/home/bot1/inline/JSON.lua")()
local token = '432026535:AAGv7F_83RzW7GpBIe7eMGPyD9MSyw6bbxI' --token
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
local SUDO = 372530129
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 295422429 or q.from.id == 372530129 then
 ...
 if redis:get(hash) then
    ...
    return false
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'[🔖Send Your Msg🔖](https://telegram.me/nice20pv_robot)',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'[✔️Send Your Problem✔️](https://telegram.me/nice20pv_robot)',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'[❗️Send Your Offer❗️](https://telegram.me/nice20pv_robot)',keyboard)
            end
							------------------------------------------------------------------------
        ...
        elseif is_lock(chat,value) then
         ...
         {text=getsettings('warn'),callback_data=chat..':lock warn'}, {text = '⬅️ عملکرد اخطار 💠', callback_data = chat..'_warn'}
          ...
          {text='⬇️',callback_data=chat..':lock WARNMAXdown'},{text='⬆️',callback_data=chat..':lock WARNMAXup'}
            ...
            else Canswer(q.id,'⚠️شما ادمین نیستید⚠️\n @Nice20Team',true)
						end
						end
